...
static void BASIC_TIM_Mode_Config(void){
    ...
    /*开启定时器时钟，即内部时钟CK_INT=72*/
    BASIC_TIM_APBxClock_FUN(BASIC_TIM_CLK, ENABLE);

    /*自动重装载寄存器的值，累计TIM_Period+1 个频率后产生一个更新或者中断*/
    TIM_TimeBaseStructre.TIM_Period = BASIC_Period;

    /*时钟预分频数为*/
    TIM_TimeBaseStructre.TIM_Prescaler = BASIC_Prescaler;
    ...
    /*初始化定时器*/
    TIM_TimeBaseInit(BASIC_TIM, &TIM_TimeBaseStructre);
